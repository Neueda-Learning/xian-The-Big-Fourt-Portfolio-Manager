...
public class AiChatService {
    ...
    public AiChatResponse chat(String model, String message) {
        ...
        String systemApiKey = StringUtils.hasText(aiProperties.getApiKey()) ? aiProperties.getApiKey().trim() : "";
        if (!StringUtils.hasText(systemApiKey)) {
            return new AiChatResponse("The system AI API key is not configured. Please contact the administrator.", normalizedProvider(), selectedModel);
        }
        ...
        headers.setBearerAuth(systemApiKey);
        try {
            ...
            if (status == HttpStatus.TOO_MANY_REQUESTS) {
                return new AiChatResponse(apiKeyUnavailableMessage(), normalizedProvider(), selectedModel);
            }
    ...
    private String apiKeyUnavailableMessage() {
        return "The system AI API key is unavailable. Please contact the administrator.";
    }
